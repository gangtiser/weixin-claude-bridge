...
import { MAX_MESSAGE_LENGTH, MIN_SEND_INTERVAL_MS, PERM_TTL_MS, CHANNEL_NAME, VERSION, sanitizeText, log, logError, errorText } from "./config.ts";
...
/** 只读校验一个待决权限请求：id 存在、未过期、回复来自被提示的会话。不消费——须在通知成功后再调 consumePendingPerm。 */
export function peekPendingPerm(requestId: string, chatId: string): boolean {
...
/** 消费（一次性）：仅在权限通知成功送达后调用。若在通知前消费，通知失败时审批会被静默丢弃且补投也救不回。 */
export function consumePendingPerm(requestId: string): void { pendingPerms.delete(requestId); }

export function createMcpServer(api: IWeixinApi, ownerId: () => string | undefined, lastPollAt: () => number = () => 0, onLogout: () => void = () => {}): Server {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
      switch (req.params.name) {
        ...
        case "wechat_history": { return txt(`【历史只读】\n` + store.readHistory(Number(a.limit) || 30).map(h => `[${new Date(h.ts).toLocaleString("zh-CN")}] ${h.direction === "in" ? h.from : "Claude"}: ${h.text.replace(/\n/g, " ⏎ ")}`).join("\n")); }
        case "wechat_status": {
          const auth = store.loadAuth(); const pend = store.listPending(); const owner = ownerId();
          return txt(JSON.stringify({
            connected: !!auth, accountId: auth?.accountId, owner: owner ? chatIdFor(owner) : undefined, pending: pend.length,
        ...
        case "wechat_logout": { store.clearAll(); onLogout(); return txt("已登出并清除凭据，桥接已停止（需重新 login 并重启会话）"); }
      }
  ...
  server.setNotificationHandler(PermReq, async ({ params }) => {
    ...
    try {
      // input_preview/description 套脱敏：这是唯一把工具输入送出本机的路径（过 iLink 服务器、落微信聊天记录），Bash 类预览常含密钥
      await api.sendMessage(ctx.senderId, `Claude 要执行 ${params.tool_name}：${sanitizeText(params.description)}\n${params.input_preview ? "输入: " + sanitizeText(params.input_preview) + "\n" : ""}回复 "yes ${params.request_id}" 或 "no ${params.request_id}"`, ctx.contextToken);
      const now = Date.now();
